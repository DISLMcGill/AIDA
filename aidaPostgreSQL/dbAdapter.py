...
class DBCPostgreSQL(DBC):
    ...
    def __init__(self, dbname, username, password, jobName, dbcRepoMgr, serverIPAddr):
        ...
        if (not AConfig.AIDAM is None):
            
            """connect to monetdb"""
            self._dbm = AIDA.connect('localhost',dbname,username,password,jobName,AConfig.AIDAMPORT);
    ...
    def _getAIDAM(self):
        #if (not self._dbm is None):
           # logging.debug("_getAIDAM: self.__dbm__ is not None");  
        return self._dbm;
    ...
    def _executeQry(self, sql, resultFormat='column', sqlType=DBC.SQLTYPE.SELECT):
        logging.debug("_executeQry is executed!")
