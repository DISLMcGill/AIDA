...
def trainingLoop(dw):
    ...
    with tf.device('/gpu:1'):
        train_set = tf.constant(normed_train_data, dtype=tf.float32, shape=[5000, 5])
        label = tf.constant(train_labels, 'float32', shape=[5000, 1])
    transfer_end = time.time()
    print(train_set.device)
    print(label.device)
    transfer_time = transfer_end - transfer_start
    logging.info('The data transfer time on GPU for a dataset of 5000 and 500 epochs using TensorFlow is:'+str(transfer_time))
    ...
    logging.info('The execution time on GPU for a dataset of size 5000 and 500 epochs using TensorFlow is:'+str(execution_time))
