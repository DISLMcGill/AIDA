...
class DBC(metaclass=ABCMeta):
    ...
    def _X(self, func, *args, **kwargs):
        ...
        if(isinstance(func, str)):
            func = super().__getattribute__(func);
        return func(self, *args, **kwargs,nn = sys.modules["torch.nn.modules"],torch = sys.modules["torch"], datasets = sys.modules["sklearn.datasets"], F = sys.modules["torch.nn.functional"],np = sys.modules["numpy"],tf = sys.modules["tensorflow"]);
