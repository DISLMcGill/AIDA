...
class ExternalDataTransform(Transform):
    ...
    def __processTransform__(self):
        ...
        logging.debug("The data is: {}".format(data))
...
class DataFrame(TabularData):
    def __init__(self, source, transform, name=None, dbc=None):
        ...
        self.__dbc__ = dbc;
        self.__dbm__ = None;
        self.__MonetData__ = None;
        self.__sentToAIDAM__ = False;
...
#
    ...
    def _hasOnlyNumPySource(self):
        if not self.__data__ is None:
            return True;
        elif self.__source__ is None:
            return False;
        else:
            if isinstance(self.__source__,TabularData):
                if isinstance(self.__source__, DBTable):
                    return False;
                if(not self.__source__._hasOnlyNumPySource()):
                    return False;
            else:
                for source in self.__source__:
                    #logging.debug("DataFrame {} : the source is {}".format(self.tableName,source.tableName));
                    if isinstance(source,DBTable):
                        return False;
                    if(not source._hasOnlyNumPySource()):
                        return False;
        return True;
     
    def _sendDataToAIDAM(self):
        if not self.__data__ is None:
            if self.__sentToAIDAM__:
                return;
            else:
                self.__dbm__ = self.dbc._getAIDAM();

                def loadFun(df):
                    return df.cdata;

                self.__MonetData__ = self.__dbm__._L(loadFun, self);
                logging.debug("DataFrame {}: ._L is called successfully!".format(self.tableName));

                self.__dbm__._toTable(self.__MonetData__,tableName = self.__tableName__);
                logging.debug("DataFrame {}: _toTable is called successfully!".format(self.tableName));
                self.__sentToAIDAM__ = True;
        else:
            if isinstance(self.__source__,TabularData):
                self.__source__._sendDataToAIDAM();
            else:
                for source in self.__source__:
                    source._sendDataToAIDAM();

    @property
    def rows(self):
        if(AConfig.SERVERTYPE=='postgres'):
            ...
            if(self.__data__ is None):
                ...
                if(self.isDBQry):
                    ...
                    #logging.debug("DataFrame {}: SQL query is: {}".format(self.tableName,sql));
                    flag = self._hasOnlyNumPySource();
                    logging.debug("DataFrame: {} : hasOnlyNumPySource called. Flag is {}".format(self.tableName,flag));
                    if (not flag) or AConfig.AIDAM is None:
                        (data, rows) = self.dbc._executeQry(sql);
                        logging.debug("DataFrame: {} : _executeQry called successfully!".format(self.tableName));
                        #Convert the results to an ordered dictionary format.
                        if(not isinstance(data, collections.OrderedDict)):
                            data_ = collections.OrderedDict();
                            for c in self.columns:
                        ...
                        self._sendDataToAIDAM();
                        logging.debug("DataFrame: {} : _sendDataToAIDAM called successfully".format(self.tableName));
                        self.__dbm__ = self.dbc._getAIDAM();
                        (data, rows) = self.__dbm__._executeQry(sql);
                        logging.debug("DataFrame: {} : _executeQry called successfully!".format(self.tableName));
                        #Convert the results to an ordered dictionary format.
                        if(not isinstance(data, collections.OrderedDict)):
                            data_ = collections.OrderedDict();
    ...
    def _loadExtData_(cls, func, dbc, *args, **kwargs):
        logging.debug("_loadExtData_ is executed!")
